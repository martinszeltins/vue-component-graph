...
type Graph = { [file: string]: Set<string> }
...
const printAscii = (entry: string, graph: Graph): string => {
    const lines: string[] = []
    lines.push(path.basename(entry))
    const walk = (node: string, indent: string) => {
...
export const generateAsciiTree = async ({ root, entry }: Options): Promise<string> => {
...
export const generateHtmlTree = async ({ root, entry }: Options): Promise<string> => {
    const allFiles = await findVueFiles(root)
    const graph: Graph = {}
    await buildGraph(entry, allFiles, graph)
